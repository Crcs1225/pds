...
// Check for JSON decoding errors or missing 'learning' field
if ($data === null || !isset($data['learning'])) {
    echo json_encode(["success" => false, "error" => "Invalid JSON format or missing 'learning' field"]);
    exit;
}

$LearningRecords = $data['learning'];

$insertedIds = [];
$errors = [];

// Loop through each learning entry
foreach ($LearningRecords as $entry) {
    // Extract values, with fallback defaults
    $personal_info_id = $entry['personal_info_id'] ?? null;
    $title = $entry['ldTitle'] ?? 'N/A';
    $ld_from = $entry['ldFrom'] ?? 'N/A';
    $ld_to = $entry['ldTo'] ?? 'N/A';
    $number_of_hours = $entry['ldHours'] ?? 'N/A';
    $type_of_ld = $entry['ldType'] ?? 'N/A';
    $conducted_by = $entry['ldConductedBy'] ?? 'N/A';
    ...
    $stmt = $con->prepare("INSERT INTO learning_and_development (personal_info_id, title,	ld_from,	ld_to,	number_of_hours,	type_of_ld,	conducted_by) 
        VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    $stmt->bind_param("issssss", $personal_info_id, $title, $ld_from, $ld_to, $number_of_hours, $type_of_ld, $conducted_by);
