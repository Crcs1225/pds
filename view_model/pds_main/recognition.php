...
// Check for JSON decoding errors or missing 'recognition' field
if ($data === null || !isset($data['recognition'])) {
    echo json_encode(["success" => false, "error" => "Invalid JSON format or missing 'recognition' field"]);
    exit;
}

$recognitionRecords = $data['recognition'];

$insertedIds = [];
$errors = [];

// Loop through each recognition entry
foreach ($recognitionRecords as $entry) {
    // Extract values, with fallback defaults
    $personal_info_id = $entry['personal_info_id'] ?? null;
    $recognition = $entry['value'] ?? 'N/A';
    ...
    $stmt = $con->prepare("INSERT INTO non_academic_distinctions (personal_info_id, recognition) 
        VALUES (?, ?)");
    ...
    $stmt->bind_param("is", $personal_info_id, $recognition);
