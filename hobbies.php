...
// Check for JSON decoding errors or missing 'hobbies' field
if ($data === null || !isset($data['hobbies'])) {
    echo json_encode(["success" => false, "error" => "Invalid JSON format or missing 'hobbies' field"]);
    exit;
}

$HobbiesRecords = $data['hobbies'];

$insertedIds = [];
$errors = [];

// Loop through each hobbies entry
foreach ($HobbiesRecords as $entry) {
    // Extract values, with fallback defaults
    $personal_info_id = $entry['personal_info_id'] ?? null;
    $skill_hobby = $entry['value'] ?? 'N/A';
    ...
    $stmt = $con->prepare("INSERT INTO special_skills_hobbies (personal_info_id, skill_hobby) 
        VALUES (?, ?)");
    ...
    $stmt->bind_param("is", $personal_info_id, $skill_hobby);
